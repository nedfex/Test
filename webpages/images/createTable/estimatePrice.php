...
function estimatePrice($TICKER,$SYMBOL , $CurrentData)
{
		...
		//current EPS
		$query = "SELECT * FROM `$ANNUAL"."_keyratio"."` WHERE `TICKER` = '$TICKER';";
		$row = mysql_fetch_array(mysql_query($query));
		$EPS = $CurrentData['EPS'];
		...
		$FINAL_PE = min( $default_PE ,$CurrentData['PE'],$CurrentData['FORWARD_PE']/*,nonNullmean($AVPE) Moring star do not have avg pe*/ ); 
		...
		$query = "SELECT `Return on Equity 9 year`,`Book Value Per Share` FROM `".($ANNUAL)."_keyratio` WHERE `TICKER` = '$TICKER';";
		$row = mysql_fetch_array(mysql_query($query));
		$AVG_ROE = $row['Return on Equity 9 year'];
