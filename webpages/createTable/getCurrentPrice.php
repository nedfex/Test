...
function getCurrentPrice($TICKER,$SYMBOL,$Update)
{	

	$query = "SELECT PRICE FROM `companyb3` WHERE `TICKER` = '$TICKER';";
	...
	//print_r($price[3]);
	$sql = "UPDATE `companyb3` SET `PRICE` = $price[3] WHERE `TICKER` = '$TICKER';";
	echo $sql;
	mysql_query($sql);
	return strip_tags(trim($price[3]));
}

function getCurrentPE($TICKER,$SYMBOL)
{	
	...
	//msn growth	
	$target = "http://investing.money.msn.com/investments/earnings-estimates?symbol=$SYMBOL";
	$web_page = http_get($target,"");
	...
	$query = "UPDATE `finance_ms`.`companyb3` SET `PRICE` = $ans[PRICE] ," ;
	if($ans['PE']!= NULL)
		$query.=" `PE` = $ans[PE] ," ;
